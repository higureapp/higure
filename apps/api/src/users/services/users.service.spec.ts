...
describe('UsersService', () => {
    ...
    describe('create', () => {
        it('should create a user if email is not taken', async () => {
            ;(usersRepository.findOneByEmail as jest.Mock).mockResolvedValue(
                null,
            )
    ...
    describe('findUnique', () => {
        it('should return public user if found and active', async () => {
            ;(usersRepository.findUnique as jest.Mock).mockResolvedValue(
                mockUser,
            )
        ...
        it('should throw NotFoundException if user not found', async () => {
            ...
            await expect(service.findUnique({ id: '1' })).rejects.toThrow(
                NotFoundException,
            )
        })
    ...
    describe('findFirst', () => {
        it('should return public user if found and active', async () => {
            ;(usersRepository.findFirst as jest.Mock).mockResolvedValue(
                mockUser,
            )
        ...
        it('should throw NotFoundException if user not found or deleted', async () => {
            ...
            await expect(
                service.findFirst({ email: 'test@example.com' }),
    ...
    describe('findOne & findOneByEmail', () => {
        ...
        it('findOneByEmail should call findUnique', async () => {
            ...
            expect(result).toEqual(mockPublicUser)
            expect(service.findUnique).toHaveBeenCalledWith({
                email: 'test@example.com',
            })
        })
    })
    ...
    describe('findOneByEmailWithPassword', () => {
        it('should return user with password', async () => {
            ...
            const result =
                await service.findOneByEmailWithPassword('test@example.com')
