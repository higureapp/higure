...
export class AnalisysService {
    ...
    async createAnalysis(
        ...
        const journal = await this.journalService.getJournalPage(journalPageId, userId);
    ...
    async regenerateAnalysis(
        ...
        const existingAnalysis = await this.analysisRepository.getAnalysisByJournalPageId(analysisId);
        if (!existingAnalysis) {
    ...
    async getAnalysisByJournalPageId(
        ...
        const analysis = await this.analysisRepository.getAnalysisByJournalPageId(journalPageId);
