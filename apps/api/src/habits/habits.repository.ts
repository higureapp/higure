...
import { Habit, HabitCategory, HabitCompletion, Prisma } from '../generated/prisma/client';
...
export class HabitsRepository {
    ...
    async findHabits(params: {
        ...
        return this.prismaService.habit.findMany({ skip, take, cursor, where, orderBy });
    }
    ...
    async createHabitCategory(data: Prisma.HabitCategoryCreateInput): Promise<HabitCategory> {
        return this.prismaService.habitCategory.create({ data });
    }

    // HabitCompletion methods
    async createHabitCompletion(data: Prisma.HabitCompletionCreateInput): Promise<HabitCompletion> {
        return this.prismaService.habitCompletion.create({ data });
    }

    async findHabitCompletionsByHabitId(habitId: string): Promise<HabitCompletion[]> {
        return this.prismaService.habitCompletion.findMany({ where: { habitId } });
    }
