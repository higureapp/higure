...
export function getNextScheduledDate(
    ...
    switch (habit.frequency) {
        ...
        case FrequencyType.SPECIFIC_DAYS_OF_WEEK: {
            const daysOfWeek = (habit.frequencyConfig as any)
        ...
        case FrequencyType.SPECIFIC_DAYS_OF_MONTH: {
            const daysOfMonth = (habit.frequencyConfig as any)
    ...
    while (true) {
        ...
        switch (habit.frequency) {
            case FrequencyType.DAILY:
                ...
                wasCompleted = completions.some((c) =>
                    isSameDay(c, expectedCompletionDate!),
                )
                break
            ...
            case FrequencyType.EVERY_N_DAYS: {
                ...
                const diffDays = Math.floor(
                    (tempCurrentDate.getTime() - habitStartDate.getTime()) /
                        (1000 * 60 * 60 * 24),
                )
            ...
            case FrequencyType.N_TIMES_PER_WEEK: {
                const { timesPerWeek, weekStartsOn } =
                    habit.frequencyConfig as any
