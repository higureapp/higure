...
export class HabitsService {
    ...
    async createHabit(
        ...
        await this.cacheManager.del(
            HabitCacheKeys.getHabits(user.userId, { isActive: true }),
        )
        await this.cacheManager.del(
            HabitCacheKeys.getHabits(user.userId, { isActive: false }),
        )
    ...
    async completeHabit(
        ...
        if (!habit || habit.userId !== userId) {
            throw new NotFoundException(
                `Habit with ID ${habitId} not found or does not belong to the user.`,
            )
        }
        ...
        const completions =
            await this.habitsRepository.findHabitCompletionsByHabitId(habitId)
        ...
        await this.cacheManager.del(
            HabitCacheKeys.getHabitStats(
                habitId,
                userId,
                StatsPeriod.WEEK.toString(),
            ),
        )
        await this.cacheManager.del(
            HabitCacheKeys.getHabitStats(
        ...
        await this.cacheManager.del(
            HabitCacheKeys.getHabitStats(
                habitId,
                userId,
                StatsPeriod.YEAR.toString(),
            ),
        )
        await this.cacheManager.del(
            HabitCacheKeys.getHabits(userId, { isActive: true }),
        )
        await this.cacheManager.del(
            HabitCacheKeys.getHabits(userId, { isActive: false }),
        )
    ...
    async getHabitStats(
        ...
        const cacheKey = HabitCacheKeys.getHabitStats(
            habitId,
            userId,
            period.toString(),
        )
        const cachedStats =
            await this.cacheManager.get<HabitStatsResponse>(cacheKey)
        ...
        while (tempDate <= endDate) {
            const completed = relevantCompletions.some((c) =>
                isSameDay(c, tempDate),
            )
        ...
        const completionRate =
            totalDaysInPeriod > 0
                ? (totalCompletions / totalDaysInPeriod) * 100
                : 0
