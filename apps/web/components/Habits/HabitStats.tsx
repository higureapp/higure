...
export function HabitStats({ habitId }: HabitStatsProps) {
  ...
  const { completionRate, currentStreak, longestStreak, totalCompletions, completionsByDay } = data?.habitStats || {};

  const chartData = completionsByDay?.map(day => ({
    ...
    <Card>
      <CardHeader>
        <div className="flex justify-between items-center">
          ...
          <Select onValueChange={(value: StatsPeriod) => setPeriod(value)} defaultValue={period}>
            <SelectTrigger className="w-[180px]">
              <SelectValue placeholder="Select period" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value={StatsPeriod.Week}>Week</SelectItem>
              <SelectItem value={StatsPeriod.Month}>Month</SelectItem>
              <SelectItem value={StatsPeriod.Year}>Year</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </CardHeader>
      <CardContent>
        ...
        <h3 className="text-lg font-semibold mb-4">Completions by Day</h3>
