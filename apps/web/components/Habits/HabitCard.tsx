...
export function HabitCard({ habit }: HabitCardProps) {
  const { mutateAsync: completeHabit, isPending: loading } = useCompleteHabitMutation();
  ...
  return (
    <>
      <Card>
        <CardHeader>
          {habit.category && (
            <Badge className="mr-2" style={{ backgroundColor: habit.category.color || '#94a3b8' }}>
              {habit.category.name}
            </Badge>
          )}
        ...
        <CardContent>
          ...
          {habit.scheduledTime && (
            <p className="text-sm text-gray-500">
              Scheduled: {new Date(habit.scheduledTime).toLocaleTimeString()}
            </p>
          )}

          <p className="text-sm text-gray-500">Streak: {habit.streakCount} days</p>
        </CardContent>
